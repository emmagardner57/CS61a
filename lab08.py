...
def insert_into_all(item, nested_list):
    ...
    [[0], [0, 1, 2], [0, 3]]
    """
    return [[item] + inside for inside in nested_list]
...
def subseqs(s):
    ...
    if s==[]:
        ...
        help=s[1:]
        return insert_into_all(s[0], help) + help
...
class PairsIterator:
    ...
    def __next__(self):
        if self.x == len(self.lst):
            raise StopIteration
        answer = (self.lst[self.x], self.lst[self.y])
...
def long_paths(tree, n):
    ...
    if n<=0 and tree.is_leaf==True:
        road.append(Link(tree.label))
    for b in tree.branches:
        for road in long_paths(b, n-1):
            road.append(Link(tree.label, road))
    return road
