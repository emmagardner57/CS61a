...
def hefty_hogs(player_score, opponent_score):
    ...
    # END PROBLEM 2
# player score and current score, 
#returns number of points HH when player rolls 0 dice (done)
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if leader==None:
        ...
        y= None
        return last_leader, y
    else:
        y = f'Player {leader} takes the lead by {x}'
        return leader, y
    # END PROBLEM 6
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    num = 0
    while count <= 10:
        roll_average = make_averaged(roll_dice, total_samples)
        av = roll_average(count, dice)
        if av > max:
            max = av
            num = count
        count+=1
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    new = score + hefty_hogs(score, opponent_score)
    if hefty_hogs(score, opponent_score) >= threshold:
        return 0
    elif hog_pile(new, opponent_score) != 0:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
