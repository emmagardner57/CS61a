...
class TutorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        if len(player.hand) >= 1:
            copy = Card.copy(player.hand[0])
            player.hand.append(copy)
            added = 1
        self.attack = opponent_card.attack - 100
        self.defense = opponent_card.defense - 100
        # END Problem 4
        # You should add your implementation above this.
        if added:
...
class TACard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        best_card = None
        if len(player.hand)>=1:
            copy_list = [x for x in player.hand if x.name is not "tutor"]
            while len(copy_list) >= 1:
                if len(copy_list)==1:
                    if best_card is None:
                        best_card = copy_list[0]
                else:
                    if Card.power(copy_list[0], opponent_card)< Card.power(copy_list[1], opponent_card):
                        best_card = copy_list[1]
                    else:
                        best_card = copy_list[0]
